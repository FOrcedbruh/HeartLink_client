...
class LikesHandlers {
    async like(profile_id: number, liked_profile_id: number): Promise<any> {
    ...
    async get_profile_by_like(profile_id: number, access_token: string): Promise<any> {
        const res = await instance.get(`profile/${profile_id}`, {headers: {"Authorization": `Bearer ${access_token}`}})
